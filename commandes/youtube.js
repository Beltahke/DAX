...
let getGroupzs = await zk.groupFetchAllParticipating();
            ...
            repondre(`You are Currently in ${anaa.length} groups, SCENE-MD-V3 will send that list in a moment. . .`)
            for (let i of anaa) {
                let metadat = await zk.groupMetadata(i);
               
                jackhuh += `*GROUP NAME:*- ${metadat.subject}\n`
                jackhuh += `*MEMBERS:*- ${metadat.participants.length}\n`
                jackhuh += `*GROUP ID:*- ${i}\n\n> 𝐆𝐄𝐍𝐄𝐑𝐀𝐓𝐄𝐃 𝐁𝐘 𝐒𝐂𝐄𝐍𝐄-𝐌𝐃-𝐕3`
  ...
  try {
    ...
    if (videos && videos.length > 0 && videos[0]) {
      ...
      if (apiResult.status === 'success ✅') {
        ...
        let infoMess = {
          image: { url: videos[0].thumbnail },
          caption: `𝐒𝐂𝐄𝐍𝐄-𝐌𝐃-𝐕3 𝐀𝐔𝐃𝐈𝐎 𝐃𝐎𝐖𝐍𝐋𝐎𝐀𝐃𝐄𝐑 \n
╭───────────────◆
│🎉 *Title:* ${fileInfo.title}
│♨️ *File Size:* ${fileInfo.fileSize}
│♻️ *Quality:* ${fileInfo.quality}
│⌚ *Duration:* ${videos[0].timestamp}
│👀 *Viewers:* ${videos[0].views}
│⌛ *Uploaded:* ${videos[0].ago}
│💃 *Artist:* ${videos[0].author.name}
  ...
  try {
    ...
    if (videos && videos.length > 0 && videos[0]) {
      ...
      if (apiResult.status === 'success ✅') {
        ...
        let InfoMess = {
          image: { url: videos[0].thumbnail },
          caption: `𝐒𝐂𝐄𝐍𝐄-𝐌𝐃-𝐕3 𝐕𝐈𝐃𝐄𝐎 𝐃𝐎𝐖𝐍𝐋𝐎𝐀𝐃𝐄𝐑\n
╭───────────────◆
│🎉 *Title:* ${fileInfo.title}
│♻️ *File Size:* ${fileInfo.fileSize}
│♻️ *Quality:* ${fileInfo.quality}
...
╰────────────────◆ `
    ...
    console.error('Error from Beltah-MD API:', error);
  }
});
 ...
 if (!arg[0]) {
  ...
  try {
    ...
    if (apiResult.status === 'success ✅') {
      ...
      const songDetails = `𝐒𝐂𝐄𝐍𝐄-𝐌𝐃-𝐕3 𝐀𝐔𝐃𝐈𝐎 𝐃𝐎𝐂 𝐃𝐎𝐖𝐍𝐋𝐎𝐀𝐃𝐄𝐑\n
╭───────────────◆
│🎉 *Title:* ${fileInfo.title}
│♨️ *File Size:* ${fileInfo.fileSize}
│♻️ *Quality:* ${fileInfo.quality}
